    ...
    *,
    ...
    for index, section in enumerate(sections):
        ...
        if not section_body and not section_title:
            continue
        # The heading is NOT echoed into the body. It was, briefly: the old
        # player kept the beat title in a bordered chrome bar that read as
        # navigation, so the content pane opened cold. The redesigned player
        # renders beat.title as a real header inside the content column, and
        # the echo turned into the same words twice, ~40px apart, on every
        # fallback card. One copy, owned by the player.
        beats.append(
            {
                "type": "explain",
                "title": section_title,
